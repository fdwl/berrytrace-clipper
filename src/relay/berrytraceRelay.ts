  ...
  private async _onHostRpc(msg: HostRpc, ws: WebSocket): Promise<void> {
    try {
      ...
      if (msg.method === 'berrytrace.takeActivations') {
        // ── 🔴 它答的是「这个标签变成过前台吗」，**不是**「用户插手了吗」 ────
        //
        // 我们开的标签一律 `active:false`，整趟作业期间它就该一直是后台的。
        // 所以它一旦变成 active，一定是**外部**造成的。
        //
        // ⚠️ 〔0902 订正〕这里原来写的是「只可能是**人去点了它**」——**过强**。
        // `chrome.tabs.onActivated` 记的是「这个窗口的活动标签换成了它」，
        // 至少三种成因不含"点进来"这个动作：
        //   · 用户关掉当前标签 ⇒ 浏览器自动激活相邻标签，可能正好是我们这个；
        //   · Ctrl+Tab / Cmd+Opt+→ 循环切过去；
        //   · 窗口或会话恢复。
        //
        // 而且**"变成前台"本身不打断自动化**（动作走 CDP，与前后台无关）。
        // 真能打断的只有页面上真实发生的输入，那是另一条通道（角标里的输入计数）。
        // ⇒ 宿主侧拿它下结论的规矩写在 `background-browser-session.ts` 的
        //   `describeActivation`：**强判据决定说不说，弱判据决定说成什么。**
        //   李博 0902 原话：「网页自动化都是 background 操作的，为何我操作会
        //   影响到这个自动化？这有问题。」—— 他是对的，报文当场被自己的证据否掉。
        //
        // 李博 0828 那句仍然成立、只是管的是另一件事：
        // 「用户如果不小心点击进来，他乱点了，会导致我们成功率的下降」——
        // 那说的是**乱点**（真实输入），不是**切进来看一眼**。
        //
